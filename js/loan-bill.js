...
function submitCommonOpinionForm(){
	$("#expenseReportForm").attr("action",webRoot+"/expense-report/expense-distributeTask.htm?opinionflag=false");
...
function selectTransactorCallback(){
	var taskId = $("#taskId").attr("value");
	var opinionflag="true";
	var userId = ztree.getId();
	$.ajax({
		data:{leaderId:userId,taskId:taskId,opinionflag:opinionflag},
		cache:false,
		type:"post",
		url:webRoot+"/expense-report/expense-distributeTask.htm",
